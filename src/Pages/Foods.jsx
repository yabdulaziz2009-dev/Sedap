...
function SkeletonCard() {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-10 flex flex-col items-center gap-3 w-60 animate-pulse shadow-sm">
      <div className="w-28 h-28 rounded-full bg-gray-100 dark:bg-slate-700" />
      ...
      <div className="flex gap-3 mt-1">
        {[...Array(4)].map((_, i) => <div key={i} className="w-6 h-6 rounded-full bg-gray-100 dark:bg-slate-700" />)}
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#f5f6fa] dark:bg-slate-900">
      ...
      <div className="flex items-center justify-between px-8 py-5">
        ...
        <div className="flex items-center gap-3">
          {/* Search Bar */}
          <div className="flex items-center gap-2 bg-white dark:bg-slate-800 rounded-full px-4 py-2.5 shadow-sm border border-gray-100 dark:border-slate-700 w-64">
            ...
            <input
              ...
              className="text-xs text-gray-600 dark:text-slate-300 bg-transparent outline-none w-full placeholder:text-gray-400 dark:placeholder:text-slate-500"
            />
          </div>

          {/* View Toggle */}
          <div className="flex items-center gap-0.5 bg-white dark:bg-slate-800 rounded-xl p-1 shadow-sm border border-gray-100 dark:border-slate-700">
            <button
              onClick={() => setViewMode("list")}
              className={`p-2 rounded-lg transition-colors ${viewMode === "list" ? "text-gray-700 dark:text-slate-200 bg-gray-100 dark:bg-slate-700" : "text-gray-300 dark:text-slate-500 hover:text-gray-500"}`}
            >
              <ListIcon />
            </button>
            <button
              onClick={() => setViewMode("grid")}
              className={`p-2 rounded-lg transition-colors ${viewMode === "grid" ? "bg-green-500 text-white" : "text-gray-300 dark:text-slate-500 hover:text-gray-500"}`}
            >
              <GridIcon />
            </button>
          </div>
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-xs font-medium transition-all ${
                    page === i + 1
                      ...
                      : "bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 hover:border-green-400"
                  }`}
                >
                  {i + 1}
                </button>
...
function PaginationBtn({ children, onClick, disabled }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-8 h-8 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-center text-gray-500 dark:text-slate-400 hover:border-green-400 disabled:opacity-30 transition-colors"
    >
      {children}
    </button>
  );
