...
import { FaArrowRight, FaEnvelope, FaEye, FaLock } from 'react-icons/fa'
import { HiMiniShieldCheck, HiSparkles, HiUserGroup } from 'react-icons/hi2'
import { clearSession, loginUser } from '../auth'

const members = ['A', 'D', 'M']
const perks = [
  {
    icon: HiMiniShieldCheck,
    title: 'Admin xavfsizligi',
    text: 'Faqat ruxsat berilgan administrator hisobi orqali tizimga kiring.',
  },
  {
    icon: HiUserGroup,
    title: 'Markazlashgan boshqaruv',
    text: 'Buyurtmalar, foydalanuvchilar va statistika bitta panelda jamlanadi.',
  },
]
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (session.role !== 'admin') {
        clearSession()
        setError("Bu hisob admin panel uchun ruxsatga ega emas.")
        return
      }
    ...
    <main className="sedap-shell px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <section className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-[1560px] items-center justify-center">
        <div className="relative flex w-full max-w-[1460px] flex-col overflow-hidden rounded-[30px] bg-white shadow-[0_28px_80px_rgba(18,24,39,0.14)] lg:min-h-[690px] lg:flex-row">
          <article className="relative flex min-h-[360px] flex-1 flex-col justify-between overflow-hidden bg-[linear-gradient(160deg,#071f39_0%,#0b3b64_52%,#082743_100%)] px-7 py-8 text-white sm:px-10 sm:py-10 lg:min-h-[690px] lg:max-w-[48%] lg:px-12">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(91,181,255,0.32),transparent_38%),radial-gradient(circle_at_bottom_left,rgba(255,255,255,0.1),transparent_30%)]" />
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3">
                ...
                <p className="sedap-display text-xl font-semibold">Kinetic Atelier Admin</p>
              </div>
              <div className="mt-8 inline-flex items-center gap-2 rounded-full border border-sky-400/30 bg-sky-400/10 px-4 py-2 text-sm font-medium text-sky-200">
                <span className="h-2 w-2 rounded-full bg-sky-400" />
                Maxsus admin kirishi
              </div>
              <h1 className="sedap-display mt-10 max-w-[420px] text-[40px] leading-[1] font-semibold tracking-[-0.06em] sm:text-[56px]">
                Admin boshqaruv markaziga kiring
              </h1>
              <p className="mt-6 max-w-[360px] text-[16px] leading-7 text-[#9cb9d5] sm:text-[18px]">
                Backenddagi admin huquqlari bilan tasdiqlangan hisob orqali panelni oching.
              </p>
            </div>
            ...
            <div className="relative z-10 mt-10 flex items-center gap-3">
              ...
              <p className="text-sm text-[#9ab8d6] sm:text-[15px]">
                Tasdiqlangan administratorlar uchun maxsus kirish oynasi
              </p>
            </div>

            <div className="relative z-10 mt-6 flex justify-between text-xs text-white/35">
              {['Boshqaruv', 'Xavfsizlik', 'Monitoring'].map((item) => (
            ...
            <div className="mx-auto flex h-full w-full max-w-[540px] flex-col justify-center">
              <header>
                ...
                <p className="mt-2 text-[16px] text-[#737b87] sm:text-[17px]">
                  Backendga ulangan admin login ma&apos;lumotlarini kiriting.
                </p>
              </header>
              ...
              >
                ...
                >
                  {isSubmitting ? 'Tekshirilmoqda...' : 'Admin sifatida kirish'}
              ...
              <footer className="mt-10 border-t border-[#ece5df] pt-9">
                <p className="text-center text-[16px] text-[#6f7681] sm:text-[17px]">
                  Oddiy foydalanuvchi sahifasiga qaytish?{' '}
                ...
                <div className="mt-6 rounded-[22px] border border-[#e8ddd5] bg-[#f7f2ee] px-5 py-4 text-center text-[15px] leading-7 text-[#5b6470] sm:text-[16px]">
                  Admin hisobingiz backendda `admin` role bilan yaratilgan bo&apos;lishi kerak.
                </div>
              </footer>
            </div>
          </section>
